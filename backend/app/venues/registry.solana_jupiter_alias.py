...
def _make_registry() -> Dict[str, VenueSpec]:
    ...
    reg: Dict[str, VenueSpec] = {
        ...
        # NEW: Solana Jupiter (Trigger/quotes) — alias to solana_dex adapter for now.
        # This is intentionally *non-trading* until the dedicated /api/dex/solana/jupiter/*
        # router + signing flow is wired end-to-end.
        "solana_jupiter": VenueSpec(
            key="solana_jupiter",
            display_name="Solana-Jupiter",
            enabled=_always_enabled,
            adapter_factory=solana_dex_factory,  # reuse on-chain adapter for reads/balances until Jupiter execution exists
