...
function marketCapRowHasData(row) {
  return [row?.price_usd, row?.market_cap_usd, row?.fdv_usd, row?.circulating_supply].some((v) => toNum(v) !== null);
}
...
function marketCapRefreshLooksFailed(rows, errors) {
  const rowList = Array.isArray(rows) ? rows : [];
  const errorList = Array.isArray(errors) ? errors : [];
  if (!rowList.length) return false;
  const anyData = rowList.some(marketCapRowHasData);
...
export default function MarketCapWindow({
  ...
  async function doRefresh(options = {}) {
    ...
    try {
      ...
      if (rows.length && marketCapRefreshLooksFailed(nextRows, nextErrors)) {
        ...
        setErrors([
          { message: "Last good market-cap snapshot retained because the refresh returned no usable metric data." },
          ...nextErrors,
        ]);
        return;
      }
      ...
      const msg = String(e?.message || e || "Market cap refresh failed");
  ...
  const visibleErrors = useMemo(() => {
    const hasUsableRows = (Array.isArray(sortedRows) ? sortedRows : []).some(marketCapRowHasData);
  ...
  const topLine = useMemo(() => {
    ...
    if (!preferred) return selectedAsset ? `${selectedAsset} cap —${knownNote}${dataNote}` : `Market cap data (${rowCounts.all} rows • ${rowCounts.owned} owned / ${rowCounts.unowned} unowned${knownNote}${modeNote}${dataNote})`;
    const asset = String(preferred.asset || selectedAsset || "—").toUpperCase();
    return `${asset} cap ${fmtMoney(preferred.market_cap_usd ?? preferred.fdv_usd, hideTableData)} • ${rowCounts.all} rows • ${rowCounts.owned} owned / ${rowCounts.unowned} unowned${knownNote}${modeNote}${dataNote}`;
