...
> **Documentation state — September 10, 2026:** this is the rolling canonical UTT README. It preserves the complete previously documented operator guide and capability history through September 3, 2026, then adds the accepted September 9 cumulative publication work and the September 10 public-repository sanitation/documentation-continuity follow-up. New release sections are additive; older capability, setup, safety, accounting, and operator guidance remains canonical unless a later section explicitly supersedes it. Runtime configuration, current Registry state, credential state, adapter capability flags, and backend safety gates remain authoritative for a specific installation.
...
## What changed in the September 9, 2026 cumulative update
...
### September 10 public-repository sanitation and documentation continuity

- The public publication manifest omits absolute local workstation paths, local usernames/machine identifiers, runtime secrets, and runtime databases.
- Developer regression tests remain part of the local DEV acceptance workflow and are intentionally excluded from the public runtime distribution.
- Runtime application source continues to publish under `backend/app/` and `frontend/src/`.
- This README is the **rolling canonical operator guide**. New cumulative release sections are additive; prior venue coverage, setup guidance, security boundaries, accounting workflows, and operator procedures remain unless explicitly superseded.
- This documentation repair restores the detailed pre-September-9 guide and layers the September 9 release material on top rather than replacing it.
